...
public class Q417Contest {
	public List<int[]> pacificAtlantic(int[][] matrix) {
		...
		for (int i = 0; i < matrix.length; i++) {
			for (int j = 0; j < matrix[i].length; j++) {
				if (canGoToAtlantic(atlanticBool, matrix, i, j) && canGoToPacific(pacificBool, matrix, i, j)) {
					int[] pos = new int[2];
					pos[0] = i;
					pos[1] = j;
					result.add(pos);
	...
	private boolean canGoToAtlantic(Boolean[][] atlanticBool, int[][] matrix, int i, int j) {
		...
		boolean up = matrix[i - 1][j] < matrix[i][j] ? canGoToAtlantic(atlanticBool, matrix, i - 1, j) : false;
		if (matrix[i - 1][j] == matrix[i][j]) {
			matrix[i][j]++;
			up = canGoToAtlantic(atlanticBool, matrix, i - 1, j);
			matrix[i][j]--;
		}
		boolean down = i < m && matrix[i + 1][j] < matrix[i][j] ? canGoToAtlantic(atlanticBool, matrix, i + 1, j)
				: false;
		...
		boolean left = matrix[i][j - 1] < matrix[i][j] ? canGoToAtlantic(atlanticBool, matrix, i, j - 1) : false;
		if (matrix[i][j - 1] == matrix[i][j]) {
			matrix[i][j]++;
			left = canGoToAtlantic(atlanticBool, matrix, i, j - 1);
			matrix[i][j]--;
		}
		boolean right = j < n && matrix[i][j + 1] < matrix[i][j] ? canGoToAtlantic(atlanticBool, matrix, i, j + 1)
				: false;
	...
	private boolean canGoToPacific(Boolean[][] pacificBool, int[][] matrix, int i, int j) {
		...
		boolean up = i > 0 && matrix[i - 1][j] < matrix[i][j] ? canGoToPacific(pacificBool, matrix, i - 1, j) : false;
		if (i > 0 && matrix[i - 1][j] == matrix[i][j]) {
			matrix[i][j]++;
			up = canGoToPacific(pacificBool, matrix, i - 1, j);
			matrix[i][j]--;
		}
		boolean down = matrix[i + 1][j] < matrix[i][j] ? canGoToPacific(pacificBool, matrix, i + 1, j) : false;
		if (matrix[i + 1][j] == matrix[i][j]) {
			matrix[i][j]++;
			down = canGoToPacific(pacificBool, matrix, i + 1, j);
			matrix[i][j]--;
		}
		boolean left = j > 0 && matrix[i][j - 1] < matrix[i][j] ? canGoToPacific(pacificBool, matrix, i, j - 1) : false;
		if (j > 0 && matrix[i][j - 1] == matrix[i][j]) {
			matrix[i][j]++;
			left = canGoToPacific(pacificBool, matrix, i, j - 1);
			matrix[i][j]--;
		}
		boolean right = matrix[i][j + 1] < matrix[i][j] ? canGoToPacific(pacificBool, matrix, i, j + 1) : false;
		if (matrix[i][j + 1] == matrix[i][j]) {
			matrix[i][j]++;
