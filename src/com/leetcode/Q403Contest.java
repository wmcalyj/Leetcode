...
public class Q403Contest {
	...
	public boolean canCross(int[] stones, int pos, int prevK) {
		...
		int lessK = k - 1;
		int moreK = k + 1;
		if (stones[pos + 1] > moreK + stones[pos]) {
			return false;
		} else {
			boolean canK = false, canLessK = false, canMoreK = false;
			int i = pos + 1;
			for (i = pos + 1; i < stones.length; i++) {
				if (stones[i] > moreK + stones[pos]) {
					break;
				} else {
					if (stones[i] == moreK + stones[pos]) {
						canMoreK = canCross(stones, i, moreK);
						if (canMoreK)
							return true;
					} else if (stones[i] == k + stones[pos]) {
						canK = canCross(stones, i, k);
						if (canK)
							return true;
					} else if (stones[i] == lessK + stones[pos]) {
						canLessK = canCross(stones, i, lessK);
						if (canLessK)
							return true;
					}
				}
			...
			return canK || canLessK || canMoreK;
		}
	}

	public static void main(String[] args) {

		Q403Contest q = new Q403Contest();
		int[][] stones = { { 0, 1, 3, 4 }, { 0, 1, 3, 4, 5, 7, 9, 10, 12 }, { 0, 1, 2, 3, 4, 8, 9, 11 },
				{ 0, 1, 3, 5, 6, 8, 12, 17 },
		...
		for (int i = 0; i < stones.length; i++) {
			System.out.println(q.canCross(stones[i]));
		}
	}
}
